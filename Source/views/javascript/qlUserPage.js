...
async function getAllUser(search = "") {
  try {
    const response = await $.ajax({
      ...
      type: "POST",
      data: { func: "getAllUser", search: search }, // Gửi từ khóa tìm kiếm
      dataType: "json",
    });
    return response;
  } catch (error) {
...
async function deleteUser(id) {
  ...
  try {
    const response = await $.ajax({
      url: "./controller/UserController.php",
      type: "POST",
      data: { func: "deleteUser", id: id }, // Gửi ID người dùng cần xóa
      dataType: "json",
    });
...
$(function () {
  ...
  $("#download-excel-templat").on("click", function () {
    window.location.href = "./assets/sample_user.xlsx"; // Đường dẫn tới file mẫu
  });
